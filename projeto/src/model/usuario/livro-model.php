<?php
class LivroModel {
    public function getLivrosMock() {
        return [
            [
                "id" => 1,
                "titulo" => "O Senhor dos Anéis",
                "autor" => "J.R.R. Tolkien",
                "status" => "Disponível",
                "imagem" => "https://covers.odilo.io/publicms/Tecnologia_e_Comunica__o__Mario_Sergio_Cortella_e_Breno_Cortella__Papo_de_Fam_lia_/VD310_318x451.jpg",
                "descricao" => "Uma épica aventura de fantasia na Terra-média."
            ],
            [
                "id" => 2,
                "titulo" => "1984",
                "autor" => "George Orwell",
                "status" => "Indisponível",
                "imagem" => "https://covers.odilo.io/publicms/Tecnologia_e_Comunica__o__Mario_Sergio_Cortella_e_Breno_Cortella__Papo_de_Fam_lia_/VD310_318x451.jpg",
                "descricao" => "Um romance distópico que explora regimes totalitários."
            ],
            [
                "id" => 3,
                "titulo" => "Dom Casmurro",
                "autor" => "Machado de Assis",
                "status" => "Disponível",
                "imagem" => "https://covers.odilo.io/publicms/Tecnologia_e_Comunica__o__Mario_Sergio_Cortella_e_Breno_Cortella__Papo_de_Fam_lia_/VD310_318x451.jpg",
                "descricao" => "Uma reflexão profunda sobre ciúmes e memórias."
            ],
            [
                "id" => 4,
                "titulo" => "Harry Potter e a Pedra Filosofal",
                "autor" => "J.K. Rowling",
                "status" => "Indisponível",
                "imagem" => "https://covers.odilo.io/publicms/Recep__o_e_conex_o__a_arte_de_cuidar_e_encantar_com_Ang_lica_Furtado/cachola_cast_8_318x451.jpg",
                "descricao" => "O início da saga mágica do jovem bruxo Harry Potter."
            ],
            [
                "id" => 5,
                "titulo" => "O Hobbit",
                "autor" => "J.R.R. Tolkien",
                "status" => "Disponível",
                "imagem" => "https://covers.odilo.io/publicms/Tecnologia_e_Comunica__o__Mario_Sergio_Cortella_e_Breno_Cortella__Papo_de_Fam_lia_/VD310_318x451.jpg",
                "descricao" => "Uma aventura fantástica do hobbit Bilbo Bolseiro."
            ],
            [
                "id" => 6,
                "titulo" => "A Revolução dos Bichos",
                "autor" => "George Orwell",
                "status" => "Disponível",
                "imagem" => "https://covers.odilo.io/publicms/Tecnologia_e_Comunica__o__Mario_Sergio_Cortella_e_Breno_Cortella__Papo_de_Fam_lia_/VD310_318x451.jpg",
                "descricao" => "Uma fábula satírica sobre política e poder."
            ],
            [
                "id" => 7,
                "titulo" => "O Pequeno Príncipe",
                "autor" => "Antoine de Saint-Exupéry",
                "status" => "Indisponível",
                "imagem" => "https://covers.odilo.io/publicms/Tecnologia_e_Comunica__o__Mario_Sergio_Cortella_e_Breno_Cortella__Papo_de_Fam_lia_/VD310_318x451.jpg",
                "descricao" => "Um clássico sobre amizade e imaginação."
            ],
            [
                "id" => 8,
                "titulo" => "Moby Dick",
                "autor" => "Herman Melville",
                "status" => "Disponível",
                "imagem" => "https://covers.odilo.io/publicms/Recep__o_e_conex_o__a_arte_de_cuidar_e_encantar_com_Ang_lica_Furtado/cachola_cast_8_318x451.jpg",
                "descricao" => "A perseguição obsessiva a uma lendária baleia branca."
            ],
            [
                "id" => 9,
                "titulo" => "Orgulho e Preconceito",
                "autor" => "Jane Austen",
                "status" => "Disponível",
                "imagem" => "https://covers.odilo.io/publicms/Recep__o_e_conex_o__a_arte_de_cuidar_e_encantar_com_Ang_lica_Furtado/cachola_cast_8_318x451.jpg",
                "descricao" => "Um clássico da literatura romântica inglesa."
            ],
            [
                "id" => 10,
                "titulo" => "O Código Da Vinci",
                "autor" => "Dan Brown",
                "status" => "Indisponível",
                "imagem" => "https://covers.odilo.io/publicms/Recep__o_e_conex_o__a_arte_de_cuidar_e_encantar_com_Ang_lica_Furtado/cachola_cast_8_318x451.jpg",
                "descricao" => "Um thriller que mistura arte, religião e conspirações."
            ]
        ];
    }
}
